...
# function kernel_gaussian(x::T; σ::T) where {T <: Real}
#     exp(x^2 / (- T(2) * σ ^ 2)) / (σ * sqrt(T(2) * π))
# end

@deprecate spike_filter(spk::Vector{T}, proj, kernel::Function; norm::Bool=true) where {T <: Real} Stats.spike_filter(spk, proj, kernel)
@deprecate spike_gaussian_filter(spk::Vector{T}, proj; σ=0.005, kwargs...) where {T <: Real} Stats.spike_filter_gaussian(spk, proj; σ)